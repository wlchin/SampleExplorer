import pytest
import pandas as pd
from ..biorag.rag_embedding import Rag_embedding
from ..biorag.transcriptome_embedding import Transcriptome_enrichment
...
def transcriptome_enrichment_instance():
    ...
    return Transcriptome_enrichment("workflow/data/study_counts.dat", 
                         ...
                         "workflow/data/important_data_for_LLM/transcription_index.p")
...
def test_get_closest_semantic_studies(rag_embedding_instance: Rag_embedding[str, str]):
...
def test_mutligeneset_construction(transcriptome_enrichment_instance: Transcriptome_enrichment[str, str, str]):
    # Test if the method returns the expected DataFrame structure
    result_df = transcriptome_enrichment_instance.list_to_dc_geneset_dictionary(test_dict)
    assert isinstance(result_df, pd.DataFrame)
    # Add more assertions based on the expected structure of the result DataFrame
    
def test_mutligeneset_construction_geneset_dim(transcriptome_enrichment_instance: Transcriptome_enrichment[str, str, str]):
    # Test if the method returns the expected DataFrame structure
    result_df = transcriptome_enrichment_instance.list_to_dc_geneset_dictionary(test_dict)
    assert result_df.shape == (10,2)
    # Add more assertions based on the expected structure of the result DataFrame
    
def test_enrichment_list_format(transcriptome_enrichment_instance: Transcriptome_enrichment[str, str, str]):
    # Test if the method returns the expected DataFrame structure
...
def test_enrichment_dict_formats(transcriptome_enrichment_instance: Transcriptome_enrichment[str, str, str]):
    # Test if the method returns the expected DataFrame structure
    res = transcriptome_enrichment_instance.run_decouplr_on_memmaped_adata(test_dict, series_list_test)
    assert res[0].shape == (12, 2)
    # Add more assertions based on the expected structure of the result DataFrame
    
def test_enrichment_samples_genelist_and_samples(transcriptome_enrichment_instance: Transcriptome_enrichment[str, str, str]):
    # Test if the method returns the expected DataFrame structure
    res = transcriptome_enrichment_instance.run_decouplr_on_memmaped_adata_with_samples(test_list, samples_list_test)
    assert res.shape == (2, 3)
    # Add more assertions based on the expected structure of the result DataFrame
    
def test_enrichment_samples_genedict_and_samples(transcriptome_enrichment_instance: Transcriptome_enrichment[str, str, str]):
    # Test if the method returns the expected DataFrame structure
